...
export default function ProductReview({product}) {
    ...
    const alreadyReviewed = product.product.reviews?.find((el) => el.user === user._id)
  ...
  return (
      ...
      <div className=' flex flex-col md:flex-row gap-5'>
        <div className=' relative h-[8rem] w-full md:w-[30%]'>
          <Image className='absolute object-contain' src={`${process.env.NEXT_PUBLIC_BACKEND_URL}/${product.product.images?.[0]}`} alt='image' fill />
        </div>
        <div className=' w-full md:w-[75%]'>
          <h1>{product.product.name}</h1>
          <p>${product.product.price}</p>
          <p className=' line-clamp-3'>{product.product.description}</p>
        </div>      
      </div>
